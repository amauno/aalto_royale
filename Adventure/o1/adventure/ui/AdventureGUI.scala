...
object AdventureGUI extends SimpleSwingApplication {
  ...
  def top = new MainFrame {
    ...
    this.contents = new GridBagPanel { 
      layout += new Label("Location:") -> new Constraints(0, 0, 1, 1, 0, 0, NorthWest.id, Fill.None.id, new Insets(8, 5, 5, 5), 0, 0)
      ...
      layout += turnOutput             -> new Constraints(1, 2, 1, 1, 1, 1, NorthWest.id, Fill.Both.id, new Insets(5, 5, 5, 5), 0, 0)
    }
