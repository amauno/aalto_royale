...
class Area(var name: String, var description: String) {
  ...
  var esineet = Map[String, Item]()
    
  private val neighbors = Map[String, Area]()
  
  
  def addItem(item:Item) = {
    esineet += item.name -> item
  }
  ...
  def fullDescription = {
    ...
    val tavarat = "\nYou see here: " + this.esineet.keys.mkString(" ")
    if (this.esineet.isEmpty) this.description + exitList
    else this.description + tavarat + exitList
  } 
